...
`Tools.autoSettle` additionally computes a `population` figure per placed settlement (base range by tier — capital/port, town, village — × a coastal/slope suitability factor already used for placement × a capital-proximity bonus read from any existing `kind:'state'` objects' `capital` points) and tags each settlement with `Tools.cultureAt(x,y)` (a point-in-polygon test over `kind:'culture'` objects) for future use. Population works whether or not states have been generated yet (the proximity bonus is neutral at 1 if none exist), so the two features have no required ordering. Each settlement is also **named** from
`Tools.cultureAt(x,y)`'s culture — the syllable pool of the culture region it stands in, so settlements inside one
culture share a naming flavour and the culture map becomes visible in the place names rather than only in the
polygons; with no culture map drawn, `cultureAt` returns null and the generator falls back to the first culture.
The name always lands on the symbol object (`o.name`, which the selection panel and the GeoJSON export read);
writing it onto the map as a label is opt-in (`App.settlegen.labels`), because an automatic label for every
settlement is a lot of ink. When that option is on, the symbol and its label are pushed as one
`History.pushCombo` step — undoing them separately would tear a name off its settlement.
...
Religions reuse this machinery wholesale. `Tools._growRegions(opts)` is the shared core the three political generators were factored down to: sample the landmass into the cheap N×N grid, reject-sample separated seeds, run one multi-source Dijkstra from all of them, trace the settled owner grid into polygons. States pass an elevation penalty and per-state `growth` multipliers; cultures and religions pass neither and differ only in seed count, separation and colour, so both sit on a second thin wrapper (`_growNamedRegions`). That removed the duplicated heap + flood-fill that `generateStates`/`generateCultures` each carried, and made `generateReligions` a six-line addition rather than a third copy. `Cv.politicalMode` now has three values (`state` | `culture` | `religion`); `state` remains the default and is what plain hand-drawn territories belong to, so the three views never bleed into each other.

Provinces take a different route on purpose. States grow *outward* over land, so they use the Dijkstra core; a province instead subdivides the *inside* of an existing state, so `Tools.generateProvinces` reuses the city generator's `_subdivide` (OBB long-axis splitting) on the state's own polygon — no landmass resampling needed, because the state polygon is already land-bounded. One geometric caveat drove a real filter: state borders follow coastlines and are frequently **concave**, and Sutherland-Hodgman clipping a concave subject can return pieces joined along the cut line whose centroid lands outside the state. Any piece whose centroid is not inside the parent is dropped, which is what keeps the invariant "a province never escapes its state" true rather than merely usually-true (`run-states-culture-test.mjs` asserts it). Provinces reuse the parent's colour at lower opacity and are excluded from the political legend, since a legend row repeating the parent's swatch carries no information.
