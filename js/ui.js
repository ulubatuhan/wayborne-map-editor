 ...
 o_front:'Mettre au premier plan', o_back:"Mettre à l'arrière-plan",
  ...
  var UI = {
    ...
    bindOptions: function () {
      ...
      self.range('st-count', 'v-st-count', function (v) { App.stategen = App.stategen || {}; App.stategen.count = Math.round(v); },
                 function (v) { return String(Math.round(v)); });
      self.range('st-variety', 'v-st-variety', function (v) { App.stategen = App.stategen || {}; App.stategen.variety = v/100; },
                 function (v) { return Math.round(v) + '%'; });
      self.range('cu-count', 'v-cu-count', function (v) { App.stategen = App.stategen || {}; App.stategen.cultureCount = Math.round(v); },
                 function (v) { return String(Math.round(v)); });
      on('btn-stategen', 'click', function () {
        var count = (App.stategen && App.stategen.count) || 5;
        var variety = (App.stategen && App.stategen.variety === undefined) ? 0.35 : App.stategen.variety;
        var n = Tools.generateStates(count, variety, Math.floor(Math.random()*4294967296));
        if (n) { Cv.politicalMode = 'state'; $('pol-mode').value = 'state'; self.msg(n + ' ' + self.t('m_stategen')); }
      });
      on('btn-culturegen', 'click', function () {
        var count = (App.stategen && App.stategen.cultureCount) || 4;
        var n = Tools.generateCultures(count, Math.floor(Math.random()*4294967296));
